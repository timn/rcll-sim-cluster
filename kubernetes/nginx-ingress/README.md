...
These files are from the [ingress-nginx](https://github.com/kubernetes/ingress-nginx) project.
They are slightly modified to fit our scenario, in particular, we use hostPort statements
in with-rbac.yaml and a nodeSelect to run the ingress controller on a specific host to
which connections from the external network are possible.

This has been used on Kubernetes 1.9 with flannel 0.10.0. This is important
because only in this (or a newer) version does the CNI configuration include
the portmap driver which is necessary to make hostPort statements work. If
you choose a different network fabric your mileage may vary. 
