...
kubectl create secret generic ceph-secret-admin --from-literal=key="$(cat ceph-client-key)" --type=kubernetes.io/rbd --namespace=kube-system
kubectl create secret generic ceph-secret-admin --from-literal=key="$(cat ceph-client-key)" --type=kubernetes.io/rbd --namespace=ceph
cd ..
